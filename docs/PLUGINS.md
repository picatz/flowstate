...
Three files, one of which is generated by `go get`.

```console
$ mkdir flowstate-plugin-hello && cd flowstate-plugin-hello
$ go mod init example.com/flowstate-plugin-hello
$ go get github.com/picatz/flowstate@latest
go: added github.com/picatz/flowstate v0.0.0-20260821212057-c4ead7c54469
```

That version is a pseudo-version of the default branch, because this repository
has no tags — see [what you are depending on](#2-what-you-are-depending-on-is-not-versioned-yet)
before you build anything you intend to keep.
...
Build it under the name discovery looks for, and ask what a worker would find:
...
There are no tags on this repository, so `go get github.com/picatz/flowstate@latest`
resolves to a pseudo-version of whatever the default branch was at that moment
