...
| `--insecure-allow-plaintext-listener` | `bool` | `false` | — | allow the public listener to serve plain HTTP on a non-loopback address with no certificate configured (overrides FLOWSTATE_INSECURE_ALLOW_PLAINTEXT_LISTENER); for a deployment whose reachability is already bounded another way. Never a fallback for a missing certificate — say it out loud, the way --insecure-no-auth is said out loud |
