...
| `--webhook <string,...>` | `stringArray` | — | — | serve deliveries for the webhooks declared in this Flowfile, at /webhooks/<workflow>/<trigger>. Repeatable. The file is compiled and its `verify:` keys are resolved at startup, so a workflow this deployment cannot serve stops the server rather than refusing deliveries later. Needs the --secret-* flags that reach the signing keys |
