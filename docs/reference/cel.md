...
| `lists` | `reverse` | function | — | `list(<T>).reverse() -> list(<T>)` |
...
data. An expression that exceeds the budget fails the step it is written on,
the same as any other evaluation error — the worker is never at risk, only
the run.
