...
record. Raw retained values are additionally bounded to 8 MiB before encoded
forms are built. If either bound is reached entirely by in-flight values, the
host suppresses plugin-controlled log text for that process rather than evicting
a value that can still leak. A manifest may additionally put an input in
