...
| Cancellation scopes | per-step `undo:` — saga compensation, run in reverse registration order when a step fails and the run cannot continue, and again when `flow cancel` stops one part way through **(done)**; spelled `undo:` and not `on_failure:` because a step's compensation runs when that step *succeeded* and a later one did not — see [DSL.md](DSL.md). The cancelled half is what this row is named for: every activity scheduled on a cancelled workflow context is refused, so those compensations run on `workflow.NewDisconnectedContext` (`context.WithoutCancel` locally) under `v1.UndoBudget`, and the run still ends CANCELED. `flow terminate` compensates nothing and cannot — it executes no workflow code. Still open: `undo:` inside a `for_each` or `parallel:`, which needs an ordering key both drivers agree on |
