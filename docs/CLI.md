...
A directory var may not stand in for a secret (#2080). Which vars are secret is decided per suite, from that suite's own `secrets:`, while a var this file states is read by every suite beside it — and none of them can see what another suite called a secret. So a suite whose `secrets:` reaches a directory var, directly, through one of its own vars, or through a leaf of a structured one, is refused at load with the chain named: `vars.token is stated by testdefaults.yaml but is on a path to a secret: vars.token → vars.alias, which tests[0].secrets["env:TOKEN"] references`. The remedy is a move, not a copy: state that var in the suite's own `vars:` and remove it from `testdefaults.yaml`, so the value lives only in the one file that withholds it. A suite that restates the directory's var with the same YAML value, written the same way, leaving the directory's copy in place, is refused the same way, because every other suite would still read and print the copy. The comparison is of the YAML as written, so it does not catch a computed copy (`${"…"}`), a scalar of a different type (`48151623` against `"48151623"`), a structured var that differs elsewhere, or a list in another order; a value the directory means to share as a secret needs the declaration #2107 proposes. Until the value moves, a sibling suite that reads the same directory var still prints it, since nothing that suite can see calls it a secret; and a secret-holding value several suites share has no spelling yet.
