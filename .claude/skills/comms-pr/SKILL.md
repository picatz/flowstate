...
comment, review comment, review reply) ends with the canonical footer,
exactly:
...
read more quietly, such as `<sup>Generated by ...</sup>`. The footer is
mandated, and the server appends it too, stripping a duplicate only when it
matches the canonical form byte for byte. A compact variant does not replace
the canonical footer; it stacks with it, and the post ends up with two
footers where there was one. Where the repo controls the surface and no
mandate applies, generated docs and artifact pages, the compact form
