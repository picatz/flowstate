...
mechanism when it is subtle, the proof when the change claims one. A
truly obvious change earns a subject and nothing more.

Footers, stated here because no repo file carries them: a commit authored
by a Claude agent ends with a trailer naming the acting model, in exactly
this form:

    Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>

GitHub posts (PR bodies, issue and review comments) are a different
surface with a different marker; they end with the italic attribution
line:
