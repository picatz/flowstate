...
func CheckTaskCapabilitiesAvailable(required, available []string) error {
	...
	// neither that the task is unregistered nor what to do about it.
	return NewTaskError(missing[0], ErrorKindUnknownTask, fmt.Errorf(
		"unknown task %q: required task capabilities are unavailable: %s; register it before running (Tasks.Register in an embedding program, a plugin directory on a worker), or validate the workflow first to have the step that names it pointed out",
