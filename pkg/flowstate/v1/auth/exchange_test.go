...
func TestTokenExchangeAppliesThePerTargetLifetimeCeiling(t *testing.T) {
	...
	t.Run("a target naming no ceiling takes the default, not the hard bound", func(t *testing.T) {
		...
		// And the default admits a token that reaches it exactly, which is the
		// direction a zero ceiling would break while the refusal above still
		// passed for the wrong reason.
		credential, err = exchange(t, 0, int(auth.DefaultMaxCredentialLifetime.Seconds()))
		require.NoError(t, err)
...
func TestTokenExchangeReadsGrantedScopesAsASet(t *testing.T) {
	...
	t.Run("the same scopes in another order are the same grant", func(t *testing.T) {
		...
		require.ElementsMatch(t, []string{"read", "write"}, credential.Scopes,
			"and the credential records the set that was granted")
	})
...
// that also refuses answers the RFC permits is not strict, it is broken.
//
// The other two responses a conforming server is entitled to send have tests of
// their own, because each also has a direction that must still be refused:
// TestTokenExchangeReadsGrantedScopesAsASet for a reordered scope grant, and
// TestTokenExchangeAppliesThePerTargetLifetimeCeiling for a lifetime inside the
// ceiling this target configured.
