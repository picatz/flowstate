...
var functionCandidates = sync.OnceValue(func() (out []refCandidate) {
	...
	for _, fn := range v1.ProfileFunctions(v1.CurrentProfile) {
		...
		// A macro's Name is not its call form, which [v1.LibraryFunction] says in as
		// many words: cel-go identifies a macro by the name after the dot, so
		// `math.greatest(1, 2)` arrives here called `greatest`. Offered as a bare
		// name it inserted `greatest`, and every one of the twelve macros the profile
		// adds answered `no function called "greatest"` from the validator on the
		// very next keystroke — the editor completing a name into a diagnostic.
		//
		// Where a macro *is* writable is decided from its example rather than from
		// its name, because the name cannot tell the two shapes apart. See
		// [macroQualifier].
		if fn.Macro {
	...
	slices.SortFunc(namespaces, func(a, b refCandidate) int { return strings.Compare(a.name, b.name) })
...
func namespaceCandidate(qualifier string) refCandidate {
	return refCandidate{
		name:   qualifier,
		detail: "functions",
...
// [v1.LibraryFunction] says so and provides the example that does. `greatest` is
// written `math.greatest(1, 2)`, on a namespace; `sortBy` is written
// `[3, 1, 2].sortBy(v, v)`, on a value. The first belongs in a namespace's member
// list. The second belongs after a dot on an expression, which this package has no
// completion surface for — better absent than offered in a spelling that does not
// compile.
//
// Decided by asking whether what precedes the call in the example is a plain
// identifier. `math` is; `[3, 1, 2]`, `{'a': 1}` and `optional.of(2)` are not, the
// last one because a namespace is a name and not a call.
...
func functionsAfter(qualifier string) []refCandidate {
	...
	for _, fn := range v1.ProfileFunctions(v1.CurrentProfile) {
		...
		if fn.Macro {
			// The other half of the same correction. A macro's name has no qualifier
			// in it, so matching on the name alone left `math.greatest` — a spelling
			// the validator accepts — offered nowhere at all, while the spelling it
			// rejects was offered at the top level.
...
func functionDocs(fn v1.LibraryFunction) string {
	...
	// The example rather than a general remark about where macros are written. It is
	// the whole reason a macro's entry needs prose at all: its name is not its call
	// form, and the two macros with no example are the two nobody can write anyway.
	if fn.Example != "" {
