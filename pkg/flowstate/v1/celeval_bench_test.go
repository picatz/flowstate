...
// holds the local driver's per-step cost on a straight chain; the other four
// are in flowfile (parse-and-validate, and marshal), in cmd/flow/internal/mcp
// (the descriptor to JSON-Schema projection), and in secrets (redacting one
// HTTP response body, which every http step pays before its reply can be
// looked at).
