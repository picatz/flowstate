...
func TestSumFailsTheWayTheOperatorDoes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// A map fold would combine keys, not values, under CEL's comprehension
		// contract. Refuse that surprising interpretation and make the author
		// select values explicitly; celmap.go separately makes the traversal
		// itself canonical.
