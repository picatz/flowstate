...
//
// An image the kernel runs through an *interpreter* rather than directly takes
// that same path, on the platform that can otherwise pin: see the format gate in
// pinToDescriptor. Those are shapes [Discover] accepts, so they keep working
// here, at the guarantee this can actually make about them.
