...
//
// Config.CallTimeout is the operator-controlled upper bound on plugin work.
// The caller may impose a shorter deadline (for example, from a step's
// `timeout:`), but an author-controlled deadline must not widen the host's
// resource bound. context.WithTimeout preserves the earlier of the two.
