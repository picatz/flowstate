...
//
// Every request is subject to the deployment's egress policy; a workflow
// reaches only hosts that policy allows.
...
type Task_HTTP_Inputs struct {
	...
	//
	// A header value may be a secret reference; such a mapping is carried as
	// a structure among the task's inputs instead of this field, and each
	// reference is resolved inside the activity only as its header is set.
	...
	// Outputs selects what the step returns. When set, the step's outputs are
	...
	// A structured request body, serialized as JSON and sent with
	// `Content-Type: application/json` unless `headers` sets one. Mutually
	// exclusive with `body` and `form`.
	//
	// Fields may hold secret references at any depth: the task serializes the
	// body inside the activity, so a reference is resolved only as the bytes
	// are written.
