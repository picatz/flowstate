...
// dispatches from it, the validator checks against it and the editor completes
// from it. This is that registry's machine-readable form, the same document
// `flow tasks --output json` prints and GetCatalog returns.
type TaskCatalog struct {
	...
	// of `if:`, `items:`, `wait_until:`, a `vars:` binding and a task input alike.
	...
	// then by name, with each one's call form and description.
	//
	// Not listed: CEL's standard functions and macros (`size`, `has`, `filter`
	// and the rest), which every expression has, and the duration constructors,
	// which are in `duration_units`.
...
type CELFunction struct {
	...
	//
	// cel-go does not say whether a macro's `name` is written on a namespace or
	// on a value: `greatest` is written `math.greatest(1, 2)` and `sortBy` is
	// written `[3,1,2].sortBy(v, v)`. This fills that gap. An ordinary function
	// does not need it, because `signature` carries its call form. Every example
	// is evaluated by a test, so one that stops working fails the build.
	...
	//
	// Derived from the profile's compiled CEL environment, so a function added to
	// a library brings its signature with it.
	Signature []string `protobuf:"bytes,5,rep,name=signature,proto3" json:"signature,omitempty"`
	// Description says what the function does, in a sentence or two: its
	// result, and any bound or failure an author should know about. Empty when
	// the function's declaration carries no documentation, which is true of
	// most functions from cel-go's extension libraries.
...
func (x *CELFunction) GetDescription() string {
	if x != nil {
...
const file_flowstate_v1_catalog_proto_rawDesc = "" +
	...
	"\x15claims_schema_version\x18\a \x01(\rR\x13claimsSchemaVersion\"\xab\x01\n" +
