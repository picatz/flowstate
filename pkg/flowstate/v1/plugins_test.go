...
func TestResolvePluginsPinsCompatibleVersion(t *testing.T) {
	...
	require.NoError(t, v1.ResolvePlugins(wf, &v1.PluginCatalog{Plugins: []*v1.PluginDescription{describedPlugin("slack", "v2.1.7", "sha256:schema")}}))
...
func TestResolvePluginsRefusesMissingOldAndWrongMajor(t *testing.T) {
	for _, tc := range []struct {
		...
		{"old", &v1.PluginCatalog{Plugins: []*v1.PluginDescription{describedPlugin("slack", "v2.0.9", "sha256:schema")}}},
		{"major", &v1.PluginCatalog{Plugins: []*v1.PluginDescription{describedPlugin("slack", "v3.0.0", "sha256:schema")}}},
	} {
...
func TestReplayGuardRefusesChangedDescriptorAtSameVersion(t *testing.T) {
	wf := &v1.Workflow{PluginRequirements: []*v1.PluginRequirement{{Name: "slack", MinimumVersion: "v2.1.0"}}}
	require.NoError(t, v1.ResolvePlugins(wf, &v1.PluginCatalog{Plugins: []*v1.PluginDescription{describedPlugin("slack", "v2.1.0", "sha256:old")}}))
	err := v1.CheckResolvedPlugins(wf, &v1.PluginCatalog{Plugins: []*v1.PluginDescription{describedPlugin("slack", "v2.1.0", "sha256:new")}})
