...
func TestASessionLinkIsFoundWhereverItSits(t *testing.T) {
	...
	assert.Equal(t, []Rule{RuleAttribution}, rules(Check("a: b", conforming+
		"\nhttps://claude.ai/code/session_01EXAMPLEEXAMPLEEXAMPLE01\n", SurfaceCommit)))
	assert.Equal(t, []Rule{RuleAttribution}, rules(Check("a: b", conforming+
		"\nPicked up from https://claude.ai/code/session_01EXAMPLEEXAMPLEEXAMPLE02 earlier.\n", SurfaceCommit)),
...
func TestEachKindOfAttributionIsReportedOnce(t *testing.T) {
	...
	body := "Refs #1\n\nVerification: `go test ./...` ok\n" +
		"\n\U0001F916 Generated with [Claude Code](https://claude.com/claude-code)\n" +
		"\nhttps://claude.ai/code/session_01EXAMPLEEXAMPLEEXAMPLE02\n" +
		"\n---\n_Generated by [Claude Code](https://claude.ai/code)_\n"
...
func TestOnlyATrailingFooterIsTreatedAsTheForges(t *testing.T) {
	...
	assert.Equal(t, []Rule{RuleAttribution}, rules(Check("a: b",
		conforming+"\n_Generated by [Claude Code](https://claude.ai/code)_", SurfacePullRequest)),
		"a rule above it is what makes a trailing footer the forge's; without one it is the author's last line")
...
// A subject reaches `git log --oneline`, every listing, and the squash subject
// a merge writes, and the body walk never sees it.
func TestTheSubjectIsHeldToTheSameShapes(t *testing.T) {
	t.Parallel()

	conforming := "Refs #1\n\nVerification: `go test ./...` ok\n"

	for _, surface := range []Surface{SurfaceCommit, SurfacePullRequest} {
		assert.Equal(t, []Rule{RuleAttribution}, rules(Check(
			"review: drop the https://claude.ai/code/session_01EXAMPLEEXAMPLEEXAMPLE01 link", conforming, surface)),
			"a link inside an otherwise well-formed subject passes every other rule")
	}

	assert.Empty(t, Check("review: report attribution a message writes for itself", conforming, SurfaceCommit),
		"a subject that merely describes the rule is not carrying one")

	// A subject that is only a footer fails its shape as well, so both are
	// reported rather than the attribution finding standing alone.
	assert.Equal(t, []Rule{RuleSubject, RuleAttribution},
		rules(Check("_Generated by [Claude Code](https://claude.ai/code)_", conforming, SurfaceCommit)))
}

// AGENTS.md's prohibition names no provider, and this repository is shared by
// several agents, so the footer is recognised for each of them. The agents are
// named rather than matched generically because the repository's own generated
// files open with a `Generated by` banner (Codex, #1985).
func TestAFooterIsRecognisedWhicheverAgentWroteIt(t *testing.T) {
	t.Parallel()
	...
	for _, footer := range []string{
		...
		assert.Equal(t, []Rule{RuleAttribution}, rules(Check("a: b", conforming+"\n"+footer+"\n", SurfaceCommit)),
			"a footer is a footer whoever signs it: %s", footer)
	}

	for _, line := range []string{
		"<!-- Generated by `flow docs generate`. Do not edit; edit the source it is derived from. -->",
		"**Contents.** *Generated by `go generate ./cmd/flow/internal/reference`; edit the source.*",
		"The reference carries a `Generated by` banner derived from the task registry.",
	} {
		assert.Empty(t, Check("a: b", conforming+"\n"+line+"\n", SurfaceCommit),
			"this repository's own generated banners are not attribution: %s", line)
	}
}

// A footer does not stop being one for being written small. The archived
// house style for a surface this repository controls is the compact
// `<sup><sub>…</sub></sup>` form, so the marker may sit inside a run of HTML
// tags (Codex, #1985).
func TestAFooterWrappedInHTMLIsStillAFooter(t *testing.T) {
	t.Parallel()

	conforming := "Refs #1\n\nVerification: `go test ./...` ok\n"

	for _, footer := range []string{
		"<sup>Generated by Codex</sup>",
		"<sup><sub>Generated by [Claude Code](https://claude.ai/code)</sub></sup>",
		"<sub><i>Generated with Amp</i></sub>",
	} {
		assert.Equal(t, []Rule{RuleAttribution}, rules(Check("a: b", conforming+"\n"+footer+"\n", SurfaceCommit)),
			"a marker written small is still a claim: %s", footer)
	}

	// The comment syntax is not a tag, so the generated banners this
	// repository writes into its own files stay unmatched.
	for _, line := range []string{
		"<!-- Generated by `flow docs generate`. Do not edit; edit the source it is derived from. -->",
		"<!-- Generated by the issue #1227 pilot. Do not edit. -->",
		"<p>The page is generated by the reference builder.</p>",
	} {
		assert.Empty(t, Check("a: b", conforming+"\n"+line+"\n", SurfaceCommit),
			"not attribution: %s", line)
	}
}
