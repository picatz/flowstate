...
// denied by name, so the refusal happens before any name resolution and this
// test needs no network of its own.
const deniedHost = "vcs.example.com"

// operatorPolicyDenying is the file an operator writes to stop this plugin
// reaching one host — the shape of policy #1322 says a deployment must be able
// to express, and could not, while this plugin built its own.
const operatorPolicyDenying = "egress:\n  deny:\n    - host == \"" + deniedHost + "\"\n"

// TestAnOperatorDenyRuleStopsAVCSTask is #1322's acceptance stated as a test:
// the deployment's own --egress-policy governs a vcs.* clone on the actual
// go-git connection path, through a real launched plugin process.
//
...
func TestAnOperatorDenyRuleStopsAVCSTask(t *testing.T) {
	if testing.Short() {
	...
	var vcsLog *flowstatev1.TaskDef
	...
	if vcsLog == nil {
		t.Fatal("the launched plugin offers no vcs.log, so nothing below tests the policy")
	}

	_, err := vcsLog.Fn(t.Context(), map[string]*flowstatev1.Value{
		"url":         flowstatev1.NewValue("https://" + deniedHost + "/an/org/repo.git"),
		"max_commits": flowstatev1.NewValue(int64(1)),
	}, nil)
	if err == nil {
		t.Fatal("vcs.log cloned from a host the operator's egress policy denies")
	}
	if !strings.Contains(err.Error(), "denied by egress policy") {
		t.Fatalf("vcs.log failed for some reason other than the operator's policy: %v", err)
	}
}
