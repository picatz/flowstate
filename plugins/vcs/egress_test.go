...
// itself (#1322), so a test binary that never received a grant is a plugin that
// refuses every clone — correctly, and uselessly for the tests below, which are
// about clone bounds, diffs and error classification rather than about egress.
// What it receives is the document a worker with no --egress-policy grants,
// because that is the launch these tests stand in for.
//
