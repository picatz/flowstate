...
//  2. requires every file under examples/plugins/sql/ to be named by some
...
func TestReadmeExamplesMatchTheFilesOnDisk(t *testing.T) {
	repoRoot := repoRootFromSQLPlugin(t)

	readmePath := filepath.Join(repoRoot, "plugins", "sql", "README.md")
	...
	exampleDir := filepath.Join(repoRoot, "examples", "plugins", "sql")
	...
	for _, entry := range entries {
		...
		relPath := "examples/plugins/sql/" + entry.Name()
		if !seen[relPath] {
			t.Errorf("%s exists but no fenced block in plugins/sql/README.md names it via "+
...
// repoRootFromSQLPlugin walks up from this test's directory to the
// repository root - the directory holding both CLAUDE.md and a go.mod,
// which is unique to the root: plugins/sql has its own go.mod but no
// CLAUDE.md, so checking for both together does not stop one level too
// early inside this plugin's own module.
func repoRootFromSQLPlugin(t *testing.T) string {
	t.Helper()
