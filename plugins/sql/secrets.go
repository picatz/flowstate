...
//
// By the time a task's Fn runs, dsn holds the value the host resolved from a
// secret reference this task declared in secret_inputs and
// required_secret_inputs (main.go). The latter declaration makes the compiler
// and host refuse ordinary literals before the task is invoked. A
	...
	case *flowstatev1.Value_SecretRef:
		return "", sdk.Failed(
			"dsn reached this task still holding a secret reference; the host is supposed to " +
				"resolve every declared secret input before calling this task, so this is a " +
