...
// netpolicy's RoundTripper, the "cap belongs under the library, where no
// path it treats specially can miss it" placement CLAUDE.md's own
...
// much larger number of object bytes - highly repetitive content compresses
// extremely well and inflates back out to its original size the moment it
// is read. Wire bytes capped at 128 MiB can therefore still expand into
// gigabytes of decompressed object data entirely inside this process,
// entirely beneath the transport bound.
//
...
// writer rather than one imported type - this file is that pattern's copy
// for the same problem, kept identical in shape to plugins/vcs's own
// packbound.go. plugins/github never clones a repository at all (it speaks
// the GitHub REST API through go-github, not go-git - see its README), so
// it has no packfile to bound and carries no copy of this file.
