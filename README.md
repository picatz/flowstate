...
| `flow test [path...]` | Run a workflow's own `*.test.yaml` files: stubbed task responses in place of the real registry, scripted signals, and a virtual clock, entirely through the local driver — no network, no Temporal, and a `sleep: 24h` step resolves in well under a second. A directory is walked for `*.test.yaml`. `--output json` or `jsonl` reports what ran as data. |
