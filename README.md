...
|---|---|---|
| **Control flow** | `if:`, `retry:` with backoff, `continue_on_error:`, `for_each`/`parallel:` fan-out, `value:` to name what an expression computes (read as `${steps.<id>.value}`), and `call:` to run another Flowfile as an isolated step | [DSL.md](docs/DSL.md) · [conditional-and-retry](examples/conditional-and-retry) · [fan-out-and-parallel](examples/fan-out-and-parallel) · [call-a-workflow](examples/call-a-workflow) |
