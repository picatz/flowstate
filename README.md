...
| **Triggers** | How a run starts, declared in the file: `schedule:` on a cron cadence, and `webhook:` for an inbound delivery, each binding its arguments with `with:` the way a `call:` does. `flow validate` checks those arguments against the workflow's `inputs:` before anything runs, and `flow test` replays a stored delivery offline | [scheduled-report](examples/scheduled-report) · [webhook-trigger](examples/webhook-trigger) |
