...
Flowstate is super-alpha: there is no release and no support contract. What exists is a maintainer who reads the tracker and a repository built to be verifiable.
...
What is not supported during super-alpha: compatibility across revisions for the CLI, the Flowfile edition, the protobuf API, persisted run history, or the plugin SDK; running workers on Windows (authoring works there; see [Deployment § Blockers](docs/DEPLOYMENT.md#blockers)); any hosted or managed offering.
