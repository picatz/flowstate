...
//   - cmd/flow/workerinternallistener_test.go — the worker subprocess reaching
//     the line that would bind. `startFlowWorker` returns on the "starting
//     worker" log, which main.go emits *before* `w.Start()` and before
//     `startInternalListener`, so real process start-up is still outstanding
//     when the sleep begins. The assertion is negative — nothing bound without
//     the flag — and it is only worth something if the binding had room to
//     appear first, so this waits on a process rather than holding a window
//     open. It read as reason 3 for exactly as long as the test's own comment
//     claiming the worker "is already polling" went unchecked against main.go.
//   - cmd/flow, the other two — windows, not waits, so both are reason 3 below
//     rather than reason 1. browser_test.go retries removing a profile
//     directory while the zygote, renderers and crash handler it did not launch
//     directly finish writing into it; and workershutdown_test.go holds a
//     window open in which the worker must still be alive.
...
//     until after the call returns, where the scrubber must still catch it.
//     The other two are host-side polls (waitForProcessGone, waitFor).
//     host_test.go:102 and :153 both let a call reach the plugin
...
//     workershutdown_test.go, and filed here so the bullet does not say
//     "waits" of a sleep that does not.
...
// would break. Three that answered "nothing" — a provider delay manufacturing
// a stampede, a wait's deadline and a parked reader — became bubbles, because
// each was paying real time for a *weaker* claim than a bubble makes: a
// stampede hoped for rather than guaranteed, a gate assumed reached after
// 100ms, a goroutine census carrying slack. Each is now an assertion the
// bubble makes exactly, and a new entry here is more likely to be a fourth of
// those than a fourth reason.
