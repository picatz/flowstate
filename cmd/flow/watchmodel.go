...
// The live shape of `flow watch`: the same [watchState] the plain shape folds into,
// redrawn in place instead of appended to.
//
// Two things make this worth the machinery over reprinting. A view that updates in
// place puts the change where the eye already is, rather than below where it was
// looking. And it can show what a printed line cannot: elapsed time, which is the
...
// out with it. A TUI that reports differently from a pipe is two commands.

// watchModel draws a run as it goes.
//
// The state is a pointer to the shared machine rather than a copy of its fields,
// which is the one place this departs from the value-model convention bubbletea
// invites. The alternative is a second implementation of "what changed and when do
// we stop", and the whole reason both shapes are correct is that there is only one.
type watchModel struct {
	surface *ui.UI
	poller  watchPoller
	state   *watchState

	// ctx is held so a poll in flight is cancelled with the program rather than
	// after it. tea.WithContext stops the event loop; it cannot reach into an RPC
	// this model started, and ctrl+c that waits out a request is ctrl+c that looks
	// broken.
	ctx context.Context

	interval time.Duration

	// first and latest are poll times, so elapsed is measured from the messages
	// that drive the model rather than from a clock it reads. That is what makes a
	// test able to state exactly what the view should say.
	first, latest time.Time

	// width and height are the terminal's, from bubbletea rather than from the
	// initial detection once the first resize arrives.
	width, height int
...
type watchPollMsg struct{ at time.Time }
...
// newWatchModel builds the live view.
func newWatchModel(
	ctx context.Context,
	surface *ui.UI,
	poller watchPoller,
...
// followLive draws a run until it finishes.
func followLive(
	ctx context.Context,
	surface *ui.UI,
	poller watchPoller,
	...
	// profile is the one detected for that stream, rather than left to bubbletea to
	// detect a second time and possibly differently.
	...
	if err != nil {
		// A cancelled context is the person pressing ctrl+c. It ends the program by
		// design, so reporting it as a failure to watch would turn an intentional
		// stop into a non-zero exit a pipeline acts on.
		...
		return fmt.Errorf("watching %q: %w", workflowID, err)
	}

	done, ok := final.(watchModel)
	if !ok {
		return fmt.Errorf("watching %q ended with a model of type %T, which is a bug", workflowID, final)
	}

	return watchEnding(surface, done)
}
...
// on.
func watchEnding(surface *ui.UI, model watchModel) error {
	if model.quit {
...
// something about the run instead of saying nothing for a second.
func (m watchModel) Init() tea.Cmd {
	return m.pollAfter(0)
}

// pollAfter schedules the next request.
func (m watchModel) pollAfter(d time.Duration) tea.Cmd {
	return tea.Tick(d, func(at time.Time) tea.Msg { return watchPollMsg{at: at} })
}

// fetch performs one request.
...
func (m watchModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		// Recorded as reported. The clamp and the fallback live in viewWidth, where
		// there is one of each rather than one per place a size arrives from.
	...
	case watchStateMsg:
		// A poll cut short by ctrl+c fails with a cancelled context, and connect
		// reports that as a refusal like any other. Absorbing it would record the
		// server as having stopped answering and exit non-zero on a run that is
		// fine — so the interruption is recognised as one, here, for the same reason
		// and in the same place the plain shape recognises it.
		if m.ctx.Err() != nil {
			m.quit = true
		...
		// Whether the walk ended badly travels on the state, not out through the
...
//
// Three spellings of stop and nothing else. A watch has no state to navigate — the
// step list is short and it is already all on screen — so every additional binding
// would be a thing to discover that does nothing. `q` because it is what every
// full-screen terminal program uses, `esc` because it is what people who do not know
// that press, and `ctrl+c` because a program that ignores it is a program people
// have to kill.
func (m watchModel) key(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	switch msg.String() {
	case "q", "esc", "ctrl+c":
		m.quit = true
...
// View draws the run.
//
// Everything here survives its own removal: the status is a word before it is a
// colour, a step is named before it is marked, and the elapsed time is prose. Strip
// every escape sequence and the screen still says what is happening — which is what
// a screen reader receives, and what a `script(1)` capture of a CI job contains.
//
// Nothing on screen may be wider than the terminal, and there are two ways to arrange
// that. Identifiers and marks are *trimmed*: an id too long to fit is looked up rather
// than read, a reader who wants the whole of one has `flow get`, and what matters here
// is that the line below it does not move. Prose is *wrapped*, by [watchModel.note] —
// see there for why that exception is the important one.
func (m watchModel) View() tea.View {
	...
	// The status pill and the id it belongs to, on the line the eye lands on first.
	fmt.Fprintf(&b, "%s %s\n",
		theme.Pill(statusTone(state.status), statusLabel(state.status)),
		theme.Strong.Render(state.workflowID))

	// The run id is what somebody needs in order to ask about this attempt later,
	// and it is not what they are watching for — so it is present and muted.
	if state.runID != "" {
	...
	if state.lastError != nil {
		// The reason nothing is moving, said rather than left to be inferred from a
	...
	// Trimmed once, over the whole screen, rather than per line as it is built.
	//
	// That is not only shorter: a line is a pill plus an id plus the space between
	// them, and trimming the parts to the full width puts the line over it by
	// however wide the other parts are. Measuring what is actually going to be
	// displayed is the only version of this with no arithmetic to get wrong.
	// ansi.Truncate, underneath, measures display width rather than bytes, so the
...
//
// Wrapped rather than trimmed, which is the one place this view reflows on purpose.
// A failure message is the reason somebody is looking at the screen, and truncating
// it at the right margin is how "connection refused" becomes "no Flowstate server
// answered at localhost:9233 (set --address or FLOWSTATE_ADD". Both notes sit below
// everything except the footer, so a message that grows by a line moves a line
// nobody is reading.
func (m watchModel) note(style lipgloss.Style, mark, text string) string {
	// The mark is outside the wrap and the text is indented under it, so a second
	// line lines up with the first rather than under the mark.
...
//
// Two sizes reach the model — the one detected for the stream and the one bubbletea
// reports on a resize — and both are recorded as given. The bound is applied here, at
// the single point of use, rather than at each of them: one clamp and one fallback,
// which is what stops a resize from quietly escaping a rule the initial size obeyed.
//
// ui.ClampWidth is the same answer every surface that prints uses. A repainting view
// wider than a printed table is two answers to "how wide is the text", in one program.
func (m watchModel) viewWidth() int {
	return ui.ClampWidth(m.width)
}

// elapsedLine reports how long the watch has been going.
//
// Measured from the poll messages, so it is the time this command has been watching
// rather than the run's own age — which is the honest thing to claim, since a watch
// attached to a run started yesterday knows nothing about yesterday. Named
// accordingly.
func (m watchModel) elapsedLine() string {
	if m.first.IsZero() {
		return "watching"
	}
...
// silently stops reporting progress at all.
func (m watchModel) visibleSteps() int {
	if m.height <= 0 {
...
//
// A run with two hundred completed steps has nothing to say in lines one to one
// hundred and eighty that the count does not say better, and the interesting end of
// the list is the recent end.
...
// window reads as a complete list, and a reader counting steps would be counting the
// wrong thing.
func (m watchModel) stepLines(theme ui.Theme, symbols ui.SymbolSet) string {
	steps := m.state.steps
	if len(steps) == 0 {
		return ""
	}
	...
	// The count, because the marks above are a window and this is the whole of it.
